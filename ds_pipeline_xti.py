...
def pred_w_test_data(df, tCol, mlModel, feature_list, saveDir):
    ...
    rvRatio = np.ceil(xtiConst.MULTI_DATI * 30000/np.power(10, pred_test)) / np.ceil(xtiConst.MULTI_DATI * 30000/np.power(10, tdata))
    ...
    #ax.set_xscale('log')
    #ax.set_title(' %s track: MSE=%0.4f' %  (trk[i], mean_squared_error(np.log10(np.ceil(multi * 30000/np.power(10, pred_test))), np.log10(np.ceil(multi * 30000/np.power(10, tdata))))))
    #ax.set_ylabel('Count')
    #ax.set_ylim((0,0.2))
    ...
    ax.plot([pltRange[0], pltRange[1]], [pltRange[0]/np.sqrt(xtiConst.MULTI_DATI), pltRange[1]/np.sqrt(xtiConst.MULTI_DATI)],
            c='red', linestyle='dashed', label='y/x=%0.2f' % (1.0/np.sqrt(1.36)))
