...
class runSkNn_opti:
    ...
    def __init__(self, model, x_data, t_data, **params_dict):
        ...
        #declariation
        if (model == 'reg'):
            self.reg_best = MLPRegressor()
        elif (model == 'class'):
            self.reg_best = MLPClassifier()
        else:
            print('invalid model')
    ...
    def runCv(self):
        ...
        if (self.model == 'reg'):
            ...
            self.reg_best = GridSearchCV(mlp_reg, self.params_dict, cv=5, n_jobs=-1, verbose=4)
            self.reg_best.fit(self.x_data, self.t_data)
            print('***Best Hyper Parameter***')
            self.mse_best = mean_squared_error(self.reg_best.predict(self.x_data), self.t_data)
            self.rsq_best = r2_score( self.reg_best.predict(self.x_data), self.t_data)
            print( self.reg_best.best_params_, self.mse_best, self.rsq_best )
        else:
            pass
