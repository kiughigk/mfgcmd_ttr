...
class runXgb_opti:
    ...
    def __init__(self, model, x_data, t_data, **params_dict):
        ...
        #declariation
        if (model == 'reg'):
            self.reg_best = xgb.XGBRegressor()
        elif (model == 'class'):
            self.reg_best = xgb.XGBClassifier()
        else:
            print('invalid model')
    ...
    def runCv(self):
        ...
        #refit with all data
        if (self.model == 'reg'):
            self.reg_best = xgb.XGBRegressor(**self.param_dict_best)
        else:
            self.reg_best = xgb.XGBClassifier(**self.param_dict_best)
    ...
    def mdl_train(self, x_data, t_data, **param_dict ):
        ...
        for train_index, test_index in rkf.split(x_data):
            ...
            if (self.model == 'reg'):
                self.reg = xgb.XGBRegressor(**param_dict)
            else:
                self.reg = xgb.XGBClassifier(**param_dict)
